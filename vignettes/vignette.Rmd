---
title: "IPSR: Detect Influential Obseravation for Feature Rankings"
author: "Shuo Wang"
...
vignette: >
  %\VignetteIndexEntry{"IPSR: Detect Influential Obseravation for Feature Rankings"}
...
Feature rankings are a crucial aspect of analyzing high-throughput data, particularly for bioinformatics studies. These rankings are typically obtained by calculating the marginal correlation with the outcomes. The ordered feature list reflects the importance of features, which plays a vital role in guiding subsequent research. For instance, researchers usually focus on a small subset of important features that are associated with research objectives. However, the feature ranking can be distorted by a single case. The case exerts abnormal influence on the feature ranking is termed as influential points (IPs). The presence of IPs renders the feature ranking unreliable, consequently affecting the subsequent analysis based on feature rankings.

The *IPSR* R package is specifically designed to detect influential observations for feature rankings. The method utilized in this package is based on the leave-one-out strategy, which involves comparing the rank difference between the original ranking and a new ranking that is obtained by removing a single observation (1). Such new rankings are known as leave-one-out rankings. The rank difference obtained through this comparison helps to determine the influence of the deleted observation.
...
We use a simple example where features are ranked based on t.test to demonstrate the use of *IPSR* package for IPs detection. We use *getdrop1ranks()* to derive the original ranking and leave-one-out rankings. Genes were simply ranked according to the p.values of t.test. Of note, the rank criteria is the p.value if *fun = "t.test"*. P.values were ranked in ascending order by specifying *decreasing = FALSE*. We selected the top 100 important genes in the original ranking. The returned object contains the original ranking (origRank) and a matrix of leave-one-out rankings (drop1Rank).
...
# Use *ipsr()* to detect IPs in one-step

*ipsr()* combines *getdrop1ranks()* and *sumRanks()* into one step. The output is identical to using the two-step process.

```{r}
results.ipsr <- ipsr(X, y, 
                     fun = "t.test",
                     decreasing = FALSE,
                     method = "adaptive")
...
*IPSR* package offers three visualization functions: *plot.ips()*, *plotRankScatters()*, and *plotAdaptiveWeights()*. *plot.ips()* can directly utilize the output of *ipsr()* or *sumRanks()* to creat a lollipop plot that displays the influence of each case. In Figure 1, we can see that the observation 68 (obs68) seems to be more influential on the final results. However, the difference between obs68 and the other observations is not that distinct, indicating a lower possibility of the presence of an influential observation.

```{r, fig.width = 7, fig.height = 4, fig.cap = "Figure 1, the influence scores for each observation"}
par(mar = c(4, 4, 2, 2))
plot(results.ipsr, topn = 5, ylim = c(0, 8))

```

In addition to the lollipop, *IPSR* also provides a simple visualization function *plotRankScatters()* that exhibits the distribution of rank changes using a scatter plot (Figure 2). Alike *plot.ips()*, *plotRankScatters()* simply use the output of *ipsr()* or *sumRanks()*. According to Figure 2, we can observe more rank changes in the tail side, but less changes in the head. The black points denotes the rank changes caused by the most influential case. 
...
In addtion to the provided ranking criteria in *ipsr()* or *sumRanks()*, which includes "t.test", "cox", "log2fc", and "kruskal.test". We can also use a custom function to rank features based on a specified rank criterion. To this end, we can pass the custom function to the *fun* argument.  The function should take *x* and *y* as inputs and output the rank criterion, such as p-values.

As an example, we can rank features based on the p-values obtained from the kruskal.test. We can either specify *fun = "kruskal.test"*, as this test has been implemented in the package, or define our own function to pass to *getdrop1ranks*. Both methods produce the same results.
...
*IPSR* provides three rank comparison methods: unweighted, weighted Spearman, and adaptive weights. We recommend using the adaptive weights. Here, we compare the three methods. Weighted Spearman and the adaptive weights method demonstrates similar results. But both are differnt weight the unweighted method. 

```{r, fig.width = 9, fig.height = 3, fig.cap = "Figure 5, IPs detection using three rank comparison methods"}
re.unweighted <- ipsr(X, y, 
                    fun = "t.test",
                    decreasing = FALSE,
                    method = "unweighted")
re.weighted <- ipsr(X, y, 
                    fun = "t.test",
                    decreasing = FALSE,
                    method = "weightedSpearman")
re.adaptive <- ipsr(X, y, 
                    fun = "t.test",
                    decreasing = FALSE,
                    method = "adaptive")
