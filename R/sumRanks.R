...
sumRanks <- function(origRank,
  ...
  if (method == "adaptive") {
    ...
    # remove non-changed ranks, and transform to data frame format for optim
    ...
    # kappa = 0 or 1 indicates abnormal distribution of rank changes
    # kappa may not be right for this situation
    if (kappa == 0 | kappa == 1)
    ...
    if (is.null(colnames(drop1RankWeighted)))
      colnames(drop1RankWeighted) <- 1:ncol(drop1Rank)
  ...
  if (method == "adaptive")
    out <- append(out, list("origRankWeighted" = origRankWeighted,
                            "drop1RankWeighted" = drop1RankWeighted))

  # define S3 class for visualization
