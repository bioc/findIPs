...
#' detection, origRank denotes the original ranking obtained using the whole data.
...
sumRanks <- function(origRank, drop1Rank, topN = NULL, method = c("adaptive", "weightedSpearman",
    "unweighted"), ...) {
    ...
    if (method == "adaptive") {
        ...
        # remove non-changed ranks, and transform to data frame format for
        # optim
        ...
        optf <- function(kappa, data, n) {
            ...
            with(data, -(1 - sum((wfun(x) - wfun(y))^2)/sum((wfun(x) - mean(wfun(x)))^2)))
        }

        kappa <- optim(par = 0.01, fn = optf, data = combR, n = n, method = "Brent",
            lower = 0, upper = 1)$par

        # kappa = 0 or 1 indicates abnormal distribution of rank changes kappa
        # may not be right for this situation
        if (kappa == 0 | kappa == 1)
        ...
        score <- apply(drop1RankWeighted, 2, function(x) sqrt(sum((origRankWeighted -
            x)^2)))
    ...
    if (method == "weightedSpearman") {
        ...
        n <- length(origRank)
        m <- seq_len(n)
        score <- apply(drop1Rank, 2, function(x) (x - m)^2 * (2 * n + 2 - x - m))
        score <- colSums(score)
        score <- score/sd(score)
    }


    # return S4 class objective
    if (method == "adaptive") {

        out <- new("ipsAdaptive", kappa = kappa, score = score, origRank = origRank,
            drop1Rank = drop1Rank, origRankWeighted = origRankWeighted, drop1RankWeighted = drop1RankWeighted)
    } else {

        out <- new("ipsOthers", score = score, origRank = origRank, drop1Rank = drop1Rank)
    }
